...
CREATE TABLE ProjectDynamite.SecurityLogs (
    ...
    CONSTRAINT `securitylogs_ibfk_1` FOREIGN KEY (`Username`) REFERENCES `UserAccounts` (`Username`) ON DELETE CASCADE
)ENGINE=INNODB AUTO_INCREMENT=1;
...
CREATE TABLE projectdynamite.AttendanceTracking (
  ...
  CONSTRAINT `attendancetracking_ibfk_2` FOREIGN KEY (`ClassCode`) REFERENCES `ClassRoster`(`ClassCode`) ON DELETE CASCADE
)ENGINE=INNODB AUTO_INCREMENT=1;
